#----------------------------------------------------------------------
#
# This library is for the Geekroo Raspberry Pi OLED PiCobber 
...
#      RPi     SSD1306
#      CE0   -> CS
#      GPIO6 -> RST   (to use a different GPIO set reset_pin to wiringpi2 pin no)
#      GPIO5 -> D/C   (to use a different GPIO set dc_pin to wiringpi2 pin no)
#      SCLK  -> CLK
#      MOSI  -> DATA
#      3.3V  -> VIN
#            -> 3.3Vo
...
class SSD1306:
    ...
    def __init__(self, bus=0, device=0, dc_pin=5, reset_pin=6, buffer_rows=64, buffer_cols=128, rows=32, cols=128):
        self.cols = cols
        self.rows = rows
